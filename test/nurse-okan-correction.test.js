...
assert(/rememberViewForControllerChange_\(\);\s*location\.reload\(\);/.test(appSource) && /if \(restoredView\) activeView = restoredView;\s*const initialViewLoad = switchView\(activeView\);/.test(appSource), 'M8: a controller-change reload must restore the active Nurse Okan view');
