...
async function run(){
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(api.buildProgressState_(events[0][1]).map(item=>item.complete))),[false,false,false,false],'empty daily data must keep Progress pending');
