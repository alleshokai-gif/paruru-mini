...
function createHarness(options = {}) {
  ...
  const sheet = name => {
    ...
    return {getLastColumn:()=>values[0].length,getLastRow:()=>values.length,
      getDataRange:()=>({getValues:()=>structuredClone(values)}),
      getRange:(row,col,count,width)=>({getValues:()=>Array.from({length:count},(_,r)=>Array.from({length:width},(_,c)=>values[row-1+r]?.[col-1+c]??'')),setValues:()=>{throw Error('SHEET_WRITE_FORBIDDEN');}})};
  };
  ...
  ['gas/HomeMemberPolicy.js','gas/HomeMembershipService.js','gas/DevicePairingService.js','gas/Code.js','gas/KazOsProgress.js','gas/KazOsProjects.js','gas/KazOsInbox.js'].forEach(f=>vm.runInContext(source(f),ctx,{filename:f}));
  ...
  return {ctx, props, rows, credentials, stats:()=>({writes,reads}), resetStats:()=>{writes=0;reads=0;},
